...
//////////////////////////Finds ID=0 April Tag Here And Follows It////////////////////////////////////////
...
//////////////////////////Finds ID=1 April Tag Here And Follows It////////////////////////////////////////
...
//////////////////////////Finds ID=3 April Tag Here And Stops, Turns Back, and Breaks Out of LOOP to THE second TIME LOOP <======================================= work on 
    ...
    break; //want to just go back 
  
}

//////////////////////////Work on the CODE above my man////////////////////////////////////////<===========================================================================

//////////////////////////After failing to find April Tags for a moment, it would swivel to find it////////////////////////////////////////


else if( Dloop == 300 ){ //trying to find the tag if it cant find it for 300 loops loop = loop + 1 at magnetometer line section  

   int LOOP = 0;
   master == 1;
   while( master == 1 ){ //master is the condition for swivel() and also this loop as well: swivel brings back a master == 0 if ti finds an April Tag 
    Swivel();
    if( LOOP >= 2 ){
      Dloop = 0;
      break;
    }
    else if( master == 0 ){ //master == 0 when swivel() finds a April Tag: Confirmed that it works at a smaller scale
      Dloop = 0;
      break;
    }
    LOOP = LOOP + 1;
...
//////////////////////////Cannot find April Tags so It just Goes Straight////////////////////////////////////////
      ...
      /*if(event.orientation.x<0){ //>>>>>>>>>>>>>>>>>>>>NEW LINE<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
if( (presentread-prevread) < 12 && (presentread-prevread)>4 ){ //go backwards if difference between present and previous is negative 100

  turn(); //turns around 

   while( d>=1500*loopcount-1500 ){ //if it previously traveled 1500mm
    int prev = analogRead(Color); //color senso
   

//////////////////////////FINDS ID==0 AND FOLLOWS IT BUT MAGNETOMETER IS DIFFERENT////////////////////////////////////////
...
//////////////////////////FINDS ID==1 AND FOLLOWS IT BUT MAGNETOMETER IS DIFFERENT////////////////////////////////////////
...
//////////////////////////This orientation should be correct trust////////////////////////////////////////
...
while( now.hour()>=12 && now.hour()<18 ){ //<====== changed "if" to a "while" here
  ...
  if(secondtimeloop == 0){ //rests loopcount and distance for more accurate measurement
    ...
    while( d<1500*loopcount){ //if it previously traveled 1500mm
    int prev = analogRead(Color); //color senso
...
//////////////////////////Cannot find April Tags so It just Goes Straight////////////////////////////////////////
      ...
      if( event.orientation.x < 180 && event.orientation.x > 5 ){ //if direction is more than 0, it would turn left
        Left();
        Serial.println(" Make Left ");
      }
      else if( event.orientation.x > 180 && event.orientation.x < 355 ){ //if direction is less than 0, it would turn right 
        Right();
        Serial.println(" Make Right ");
      }
      else if( (event.orientation.x >= 0 && event.orientation.x <= 5) || (event.orientation.x>=355 && event.orientation.x<=359)){
        Forward(); 
        Serial.println(" Go Straight ");
      } 
...
//////////////////////////Out of 2 of the biggest 2 LOOPS////////////////////////////////////////
...
/*if( now.hour()>=18 ){
  //finds its way to its house
  //and stops for a half second
}
while(true){
  //Stop();
  //if(now.hour()>==6) then break;
} */
}
...
void Bswivel(){
  ...
  Serial.print("Left: ");
  Serial.println(event.orientation.x);
  if(event.orientation.x>=88 && event.orientation.x<=90){
  condition = false;
  }
  }
  condition = true;
