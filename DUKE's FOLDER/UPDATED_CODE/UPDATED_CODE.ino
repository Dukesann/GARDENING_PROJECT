...
int Fast = 220; //speed of motor
int Slow = 150; //speed of motor
int Twist = 180; //speed of swivel

int prox_num = 20; //proximity to object before it stops 

int dnum = 3600000; //delay for each increment
...
void loop() {
  ...
  while ( now.hour() >= 6 && now.hour() < 12 ) { //Loop for Before Noon: Pretty Big So Dont Get Lost <====== changed "if" to "while"
    ...
    Stop();
    delay(dnum); //delay for a long period of time likely for an hour HAPPENS AT THE END
...
void Right() //changed from bright
{
  usMotor_Status = CW;
  motorGo(MOTOR_1, usMotor_Status, Fast);
  motorGo(MOTOR_2, usMotor_Status, Fast);
  motorGo(MOTOR_3, usMotor_Status, Slow);
  motorGo(MOTOR_4, usMotor_Status, Slow);
}

void Forward() //changed
{
  usMotor_Status = CW;
  motorGo(MOTOR_1, usMotor_Status, Slow);
  motorGo(MOTOR_2, usMotor_Status, Slow);
  motorGo(MOTOR_3, usMotor_Status, Slow);
  motorGo(MOTOR_4, usMotor_Status, Slow);
}

void Left() //changed from bleft
{
  usMotor_Status = CW;
  motorGo(MOTOR_1, usMotor_Status, Slow);
  motorGo(MOTOR_2, usMotor_Status, Slow);
  motorGo(MOTOR_3, usMotor_Status, Fast);
  motorGo(MOTOR_4, usMotor_Status, Fast);
}

void Back() //changed from forward
{
  usMotor_Status = CCW;
...
void RRight()
{
  ...
  motorGo(MOTOR_1, CCW, Twist);
  motorGo(MOTOR_2, CCW, Twist);
  motorGo(MOTOR_3, CW, Twist);
  motorGo(MOTOR_4, CW, Twist);
}

void LLeft()
{

  motorGo(MOTOR_1, CW, Twist);
  motorGo(MOTOR_2, CW, Twist);
  motorGo(MOTOR_3, CCW, Twist);
  motorGo(MOTOR_4, CCW, Twist);
}
void Reverse()
{
  usMotor_Status = CW;
...
void April() { //works on ID0 and ID1
